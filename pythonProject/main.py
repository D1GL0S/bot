...
lvlmax = '2277.png'
discord_icon = (150, 940)
discord_loop = (1055, 45)
white_pixel = (925, 235)
black_pixel = (890, 197)
osrs_icon = (100, 940)
...
def is_red_pixel(pixel):
    ...
    return pixel[0] > 240 and pixel[1] < 15 and pixel[2] < 15
...
def find_and_click_second_target(other_target_images, timeout=5, confidence=0.8):
    ...
    while time.time() - start_time < timeout:
        ...
        if other_target_position:
            ...
            time.sleep(0.3)  # Подождать полсекунды
            pyautogui.press('f1')
            time.sleep(0.3)  # Подождать полсекунды
            return True
        time.sleep(0.3)
...
while True:
    ...
    if not arrays or len(arrays) > 5 or green == 1:
        pyautogui.hotkey('F4')
        ...
        for array in arrays_to_process:
            ...
            if find_and_click_target(top_left_x, top_left_y, bottom_right_x, bottom_right_y, other_target_images):
                ...
                pixel_color2 = pyautogui.pixel(*black_pixel)
                ...
                for x_offset in range(-radius, radius + 1):
                    for y_offset in range(-radius, radius + 1):
                        ...
                        if pixel_color == (255, 255, 255) or pixel_color == (0, 0, 0):
                            ...
                            notification.notify(
                                title='Уведомление',
                                message=f'Белый пиксель обнаружен на координатах {x}, {y}. Программа завершает работу.',
                                timeout=10
                            )
                            exit()
                '''
